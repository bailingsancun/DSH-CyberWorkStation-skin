> **Fork 说明(dsh-cost-meter-plus @ 1.5.19-plus.2)**:在上游 1.5.19 基础上:
> 1. Provider 感知余额(DeepSeek 官方 → OpenRouter credits → 本地端点仅 token 提示 → OpenAI 说明无余额端点);余额符号跟随接口币种。
> 2. 启动时按 llm-pi-ai 配置自动同步 OpenRouter 模型价格(USD/1M,幂等只补缺)。
> 3. 侧栏峰谷条替换为「缓存命中条」:命中/未命中双段 + 今日命中率;悬停显示 今日/近7天/本月/累计 的 命中·未命中·总 tokens。
> 4. 会话行显示 总tokens·命中·未命中·输出;标题栏芯片悬停同步增加拆分行。
> 峰谷计价、切换弹窗提醒、设置页功能保留未动。
